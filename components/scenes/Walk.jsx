...
export default function Walk() {
  const animationData = useStore((state) => state.walkAnimationData);
  const appPhase = useStore((state) => state.appPhase);
  const animationSpeed = useStore((state) => state.animationSpeed);

  const isActive = appPhase === 'walk';
  const isOpaque = useCrossfade(isActive);

  return (
    <div
      className={`absolute inset-0 w-full h-full overflow-hidden pointer-events-none z-10 transition-opacity duration-500 ${
